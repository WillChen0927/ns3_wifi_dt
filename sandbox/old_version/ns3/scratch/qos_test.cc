...
int main(int argc, char *argv[]) {
    ...
    Simulator::Schedule(Seconds(10.0), [=]() mutable {
        ...
        for (int i = 0; i < ap1Stas.size() * 0.25; ++i) {
            uint32_t idx = ap1Stas[i];
            staNodes.Get(idx)->GetObject<MobilityModel>()->SetPosition(Vector(10.0, 0.0, 0.0));
            std::cout << "[10s] Moved STA " << idx << " (IP: " << staIpList[idx] << ") → AP2\n";
            movementLog << "10," << idx << "," << staIpList[idx] << ",AP2\n";
        }

        for (int i = 0; i < ap2Stas.size() * 0.5; ++i) {
            uint32_t idx = ap2Stas[i];
            staNodes.Get(idx)->GetObject<MobilityModel>()->SetPosition(Vector(0.0, 0.0, 0.0));
            std::cout << "[10s] Moved STA " << idx << " (IP: " << staIpList[idx] << ") → AP1\n";
            movementLog << "10," << idx << "," << staIpList[idx] << ",AP1\n";
        }
    });

    Simulator::Schedule(Seconds(20.0), [=]() mutable {
        std::vector<uint32_t> ap1Stas, ap2Stas;
        for (uint32_t i = 0; i < 32; ++i) {
        ...
        for (int i = 0; i < ap1Stas.size() * 0.5; ++i) {
            uint32_t idx = ap1Stas[i];
            staNodes.Get(idx)->GetObject<MobilityModel>()->SetPosition(Vector(10.0, 0.0, 0.0));
            std::cout << "[20s] Moved STA " << idx << " (IP: " << staIpList[idx] << ") → AP2\n";
            movementLog << "20," << idx << "," << staIpList[idx] << ",AP2\n";
        }
    
        for (int i = 0; i < ap2Stas.size() * 0.5; ++i) {
            uint32_t idx = ap2Stas[i];
            staNodes.Get(idx)->GetObject<MobilityModel>()->SetPosition(Vector(0.0, 0.0, 0.0));
            std::cout << "[20s] Moved STA " << idx << " (IP: " << staIpList[idx] << ") → AP1\n";
            movementLog << "20," << idx << "," << staIpList[idx] << ",AP1\n";
        }
    });
