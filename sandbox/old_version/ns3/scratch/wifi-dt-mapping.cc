...
void start_sim()
{
    ...
    std::map<uint32_t, Ptr<Node>> apNodeMap;
    ...
    for (const auto& ap : apList) {
        ...
        apNodes.Add(apNode);
        apNodeMap[ap.ap_id] = apNode;
        InternetStackHelper stack;
        stack.Install(apNode);

        MobilityHelper ap_mobility;
        Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator>();
        positionAlloc->Add(position); //VectorValue(Vector(ap.location.x, ap.location.y, ap.location.z))
        ...
        YansWifiPhyHelper phy24, phy5;
        phy24.SetChannel(channel.Create());
        phy5.SetChannel(channel.Create());

        NetDeviceContainer bridgePorts;

        if (ap.band == 1 || ap.band == 3) {
            phy24.Set("ChannelSettings", StringValue("{" + std::to_string(ap.channel_band1) + ",0,BAND_2_4GHZ,0}"));
            NetDeviceContainer dev24 = wifi.Install(phy24, mac, apNode);
            bridgePorts.Add(dev24);
            allDevices.Add(dev24);

            UdpServerHelper server24(10000 + ap.ap_id);
            auto app24 = server24.Install(apNode);
            app24.Start(Seconds(0.0));
            app24.Stop(simulationTime);
            apServerMap[ap.ap_name]["2.4G"] = DynamicCast<UdpServer>(app24.Get(0));
        }

        if (ap.band == 2 || ap.band == 3) {
            phy5.Set("ChannelSettings", StringValue("{" + std::to_string(ap.channel_band2) + ",0,BAND_5GHZ,0}"));
            NetDeviceContainer dev5 = wifi.Install(phy5, mac, apNode);
            bridgePorts.Add(dev5);
            allDevices.Add(dev5);

            UdpServerHelper server5(11000 + ap.ap_id);
            auto app5 = server5.Install(apNode);
            app5.Start(Seconds(0.0));
            app5.Stop(simulationTime);
            apServerMap[ap.ap_name]["5G"] = DynamicCast<UdpServer>(app5.Get(0));
        }

        if (bridgePorts.GetN() > 1) {
            BridgeHelper bridge;
            bridge.Install(apNode, bridgePorts);
        }
    }
}*/
